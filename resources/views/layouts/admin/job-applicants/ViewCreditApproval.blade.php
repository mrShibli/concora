                ...
                <tbody>
                    ...
                    @else
                        @foreach($depoData as $payment)
                            <tr>
                                <td class="action-icon"><a href="{{ route('applicants.paymentViewCreditApproval', ['applicant' => $applicant->id, 'payment' => $payment->id]) }}">👁️</a></td>
                                <td>{{ \Carbon\Carbon::parse($payment->applicant->created_at)->format('d M Y') }}</td>
